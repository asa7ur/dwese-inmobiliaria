...
public interface AgentRepository extends JpaRepository<Agent, Long> {
    @Query("SELECT COUNT(a) > 0 FROM Agent a WHERE LOWER(a.dni) = LOWER(:dni)")
    boolean existsAgentByDni(@Param("dni") String dni);

    @Query("SELECT COUNT(a) > 0 FROM Agent a WHERE LOWER(a.dni) = LOWER(:dni) AND a.id <> :id")
    boolean existsAgentByDniAndIdNot(@Param("dni") String dni, @Param("id") Long id);
